...
// === Conditional imports cho native-only modules ===
...
if (Platform.OS !== 'web') {
  ...
  try { NativeQRCode = require('react-native-qrcode-svg').default; } catch (e) { }
}
...
function BackdropContent() {
  ...
  if (BlurView) {
    return <BlurView intensity={10} tint="dark" style={StyleSheet.absoluteFillObject} />;
  }
  return null;
}
...
const FeePopupModal: React.FC<FeePopupModalProps> = ({
  ...
  const handleCreatePayment = async () => {
    ...
    if (!khoanThu || !amountToPay || isNaN(amountToPay) || amountToPay <= 0) {
      crossPlatformAlert('Lỗi', 'Số tiền không hợp lệ. Vui lòng nhập một số tiền lớn hơn 0.');
      return;
    }
    setStep('loading');
    try {
      ...
      crossPlatformAlert('Lỗi thanh toán', error?.response?.data?.message || 'Không thể tạo thanh toán. Vui lòng thử lại.');
      setStep('details');
    }
  };
  ...
  const handleWebViewNavigationStateChange = (navState: any) => {
    ...
    if (url.includes('payment-success') || url.includes('payment-cancel')) {
      if (url.includes('payment-success') && paymentInfo) {
        ...
        crossPlatformAlert('Thông báo', 'Bạn đã hủy thanh toán');
        onClose();
      }
    }
  };
  ...
  const checkPaymentStatus = async (paymentId: string, retriesLeft: number) => {
    if (retriesLeft <= 0) {
      setStep('details');
      crossPlatformAlert('Lỗi', 'Không thể xác nhận thanh toán tại thời điểm này. Vui lòng kiểm tra lại trạng thái trong danh sách khoản thu sau.');
      onClose();
      return;
    }
    ...
    try {
      ...
      if (status.status === 'paid') {
        crossPlatformAlert('Thành công', 'Thanh toán thành công!', [
          {
            text: 'OK', onPress: () => {
              onPaymentSuccess?.();
              onClose();
      ...
      setStep('details');
      crossPlatformAlert('Lỗi', 'Có lỗi xảy ra khi kiểm tra trạng thái thanh toán.');
      onClose();
    }
  };
  ...
  const renderQR = () => (
    <>
      ...
      <Button mode="contained" onPress={() => {
        // On web: open checkout URL in new tab instead of WebView
        if (Platform.OS === 'web' && paymentInfo?.checkoutUrl) {
          window.open(paymentInfo.checkoutUrl, '_blank');
        } else {
          setStep('webview');
        }
      }} style={styles.button} labelStyle={styles.buttonText}>
        {Platform.OS === 'web' ? 'Mở trang thanh toán' : 'Mở qua WebView'}
      </Button>
  ...
  const renderWebView = () => {
    // Web: fallback to opening URL in new tab (WebView not supported)
    if (Platform.OS === 'web') {
